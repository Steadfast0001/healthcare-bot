                                      ...
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          Container(
                                            ...
                                            decoration: BoxDecoration(
                                              color: isCompleted ? const Color(0xFF10B981).withValues(alpha: 0.15) : Colors.orange.withValues(alpha: 0.15),
...
class _PatientReportDetailScreen extends StatelessWidget {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      if (medications.isNotEmpty) ...[
        ...
        ...medications.map((med) => Container(
          ...
          child: Row(children: [
            Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: _kGreen.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.medication_outlined, color: _kGreen, size: 20)),
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(_getCategoryIcon(e.key.toString()), color: _getCategoryColor(e.key.toString()), size: 18),
                      const SizedBox(width: 8),
                      Text(
                ...
                Padding(
                  padding: const EdgeInsets.all(16),
                  child: _buildDynamicContent(e.value),
                ),
              ],
            ),
          );
  ...
  IconData _getCategoryIcon(String key) {
    ...
    if (k.contains('vital')) return Icons.monitor_heart_outlined;
    if (k.contains('activit')) return Icons.directions_run_outlined;
    if (k.contains('symptom')) return Icons.sick_outlined;
    if (k.contains('profile')) return Icons.person_outline;
    return Icons.data_usage_outlined;
  }

  Color _getCategoryColor(String key) {
    final k = key.toLowerCase();
    if (k.contains('vital')) return Colors.redAccent;
    if (k.contains('activit')) return Colors.greenAccent;
    if (k.contains('symptom')) return Colors.orangeAccent;
    if (k.contains('profile')) return Colors.blueAccent;
    return Colors.white70;
  }

  Widget _buildDynamicContent(dynamic value) {
    if (value is List) {
      if (value.isEmpty) return const Text('No records', style: TextStyle(color: Colors.white38));
      return Column(
        children: value.map((item) {
          ...
          return Text(item.toString(), style: const TextStyle(color: Colors.white70));
        }).toList(),
      );
      ...
      return Text(value.toString(), style: const TextStyle(color: Colors.white70));
    }
  }
}
