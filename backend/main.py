...
def forgot_password(
    ...
    if user:
        ...
        email_body = (
            ...
            f"<p>Use the 4-digit token below to reset your password. The token expires in {PASSWORD_RESET_TOKEN_EXPIRE_HOURS} hour(s).</p>"
            f"<h2>{token}</h2>"
        )
    ...
    return {
        "message": "If the email exists in our system, a 4-digit reset token has been sent."
    }
...
async def chat(
    ...
    # 2. Safety Guardrail classification (parallel LLM evaluation)
    ...
    active_chats = recent_chats[-3:] if len(recent_chats) > 3 else recent_chats

    profile = current_user.profile
    patient_context = [
        f"- Name: {current_user.full_name}",
        f"- Age: {profile.age if profile and profile.age is not None else current_user.age or 'unknown'}",
        f"- Gender: {(profile.gender if profile else current_user.gender) or 'unknown'}",
        f"- Country: {(profile.country if profile else current_user.country) or 'unknown'}",
        f"- City: {(profile.city if profile else None) or 'unknown'}",
    ]
    if profile and profile.allergies:
        patient_context.append(f"- Known allergies: {profile.allergies}")
    if profile and profile.known_conditions:
        patient_context.append(f"- Known conditions: {profile.known_conditions}")

    system_prompt = f"""You are an intelligent, professional, human-like AI assistant that communicates naturally, clearly, and confidently. Your communication style should feel conversational, engaging, modern, and highly helpful — similar to talking with an experienced tech consultant, mentor, and creative strategist.

### COMMUNICATION STYLE:
- Speak naturally like a real human expert.
- Be conversational, intelligent, and easy to understand.
- Use professional but friendly language.
- Explain concepts clearly and practically (including technical and medical concepts).
- Avoid robotic or overly formal responses.
- Give structured, detailed, and actionable answers in the "reply" key of your JSON output.
- Be confident and direct when giving recommendations.
- Maintain a calm, respectful, and collaborative tone.
- Adapt explanations for beginners and advanced users when necessary.
- Use headings, bullet points, examples, and step-by-step guidance inside your "reply".
- Keep responses clean and organized.

### AGENTIC TOOL USE:
You have access to the following tools:
- `get_patient_vitals`: Call this if the user asks about their vitals or if vital signs (blood pressure, heart rate, blood glucose, temperature, weight) are relevant to the query.
- `get_patient_active_medications`: Call this if the user asks about their active prescriptions, medications, or schedules.
- `get_patient_activities`: Call this if the user asks about their steps, sleep, hydration, or meals.
- `get_patient_goals`: Call this if the user asks about their health goals, target thresholds, or progress.
- `query_medical_knowledge_base`: Call this to search the clinical database for matching diseases/conditions, symptoms, severity, and advice. Pass a search keyword as the 'query' parameter (e.g. 'fever', 'chest pain').

You must invoke tools first before formulating your final response if the relevant data is needed. If you already have the tool output, synthesize it naturally in your final response.

### BEHAVIOR RULES:
- Think like a senior software engineer, system architect, UI/UX strategist, business analyst, creative director, and medical consultant combined.
- Analyze deeply before answering.
- Ask intelligent follow-up questions only when necessary.
- Suggest improvements, optimizations, and best practices proactively.
- When generating code:
  - Produce scalable, clean, production-level code.
  - Follow modern architecture and security practices.
  - Add comments where necessary.
  - Explain complex sections clearly.
- When helping with projects:
  - Think from requirement stage to deployment.
  - Consider scalability, performance, maintainability, and user experience.
- When generating prompts:
  - Create cinematic, detailed, high-quality prompts.
  - Make prompts compatible with modern AI tools.
- When explaining:
  - Use practical real-world examples.
  - Simplify difficult concepts without losing accuracy.

### RESPONSE FORMATTING:
- Inside the "reply" key of the JSON object, format your message as follows:
  - If the user discusses any symptoms or health issues:
    - Detail each identified health issue clearly and precisely.
    - Provide a section outlining what could be the potential causes.
    - Provide a section outlining the proposed solution or action plan.
  - Use clear titles and sections.
  - Use concise paragraphs.
  - Use bullet points for readability.
  - Avoid unnecessary filler text.
  - Keep answers informative and high quality.

### PERSONALITY:
- Intelligent, Creative, Strategic, Helpful, Professional, Modern, Supportive, Solution-oriented.

### NEVER:
- Sound robotic.
- Give vague explanations.
- Overcomplicate simple answers.
- Produce low-quality generic output.
- Ignore scalability, performance, or best practices.
- Provide unsafe or unethical solutions.

### CONSTRAINTS & BEHAVIOR:
1. Always output ONLY a valid JSON object matching the exact schema provided below. Do not include markdown wraps like ```json or any conversational fluff outside the JSON.
2. ROUTING & TRIAGING:
   - If the user shares medical/healthcare concerns or symptoms, set "risk_level" to the appropriate tier ("low", "medium", "high", or "emergency"), and perform clinical triaging.
   - For general queries, coding, prompts, or non-medical assistance, respond fully and helpful in the "reply" key, set "risk_level" to "low", and leave clinical arrays and safety warnings as null/empty.
   - If severe emergency indicators are present (e.g., severe chest pain, sudden numbness, heavy bleeding), set "risk_level" to "emergency", provide immediate safety steps in "warning", and tell them to seek urgent care.

### JSON SCHEMA TO RETURN:
{{
  "reply": "The primary response text to the user. Use this for general answers, chat pleasantries, coding assistance, or the bulk of your explanation.",
  "risk_level": "low | medium | high | emergency",
  "possible_conditions": ["Condition 1", "Condition 2"], 
  "recommended_action": "Clear, actionable next steps for the user (e.g., see a doctor, rest, hydrate, or technical steps). Null if not applicable.",
  "follow_up_question": "A logical next question to ask the user to better understand their situation. Null if not applicable.",
  "disclaimer": "A brief disclaimer stating you are an AI assistant. Use a medical disclaimer if medical queries are detected.",
  "warning": "Critical safety warnings or error-state notices if an immediate emergency is suspected. Null if not applicable."
}}
...
"""
    ...
    if older_summary:
        prompt_text += f"Summary of older conversation history:\n{older_summary}\n\n"
    for previous_chat in active_chats:
        prompt_text += f"User: {previous_chat.user_message}\nAssistant: {previous_chat.ai_response}\n"
    prompt_text += f"User: {patient_message}\nAssistant:"

    start_time = time.time()
    advice: StructuredAdvice | None = None
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        guardrail_future = executor.submit(_classify_safety_guardrail, patient_message)
        chat_future = executor.submit(_call_gemini_with_tools, prompt_text, current_user, db)
        
        try:
            guardrail_res = guardrail_future.result()
            is_emergency = guardrail_res.get("is_emergency", False)
            emergency_reason = guardrail_res.get("emergency_reason")
        except Exception as e:
            print(f"Error in parallel safety guardrail: {e}", flush=True)

        try:
            response_json = chat_future.result()
            if response_json:
                model_used = response_json.get("model", GOOGLE_MODEL)
                advice = _parse_structured_advice(_extract_gemini_text(response_json))
        except Exception as e:
            import traceback
            print("EXCEPTION IN CHAT GENERATION WITH TOOLS:", e, flush=True)
            traceback.print_exc()
            advice = None

    if advice is None:
        matched_sympts = _matched_symptoms_from_text(patient_message, db)
        symptom_names = [s.name for s in matched_sympts if s.name]
        emergency_matches = _detect_emergency_keywords(patient_message, db)
        advice = _build_fallback_advice(patient_message, db, emergency_matches)

    matched_sympts = _matched_symptoms_from_text(patient_message, db)
    symptom_names = [s.name for s in matched_sympts if s.name]
    is_health = _is_health_query(patient_message, db) or bool(symptom_names) or is_emergency

    advice = StructuredAdvice(
        reply=advice.reply,
        risk_level=_normalize_risk_level(advice.risk_level) if is_health else "low",
        possible_conditions=advice.possible_conditions[:3] if is_health else [],
        recommended_action=advice.recommended_action if is_health else None,
        follow_up_question=advice.follow_up_question if is_health else None,
        disclaimer=advice.disclaimer if is_health else None,
        warning=advice.warning if is_health else None,
    )

    if is_health:
        if not advice.disclaimer:
            advice.disclaimer = STANDARD_DISCLAIMER
        
        # Safety guardrail emergency override
        if is_emergency:
            advice.risk_level = "emergency"
            if emergency_reason:
                advice.warning = f"EMERGENCY WARNING: {emergency_reason}. Please seek immediate medical assistance."
            else:
                advice.warning = _warning_for_risk("emergency")
            advice.recommended_action = "Go to the nearest emergency room or dial emergency services immediately."

    reply_text = _build_reply_text(advice)
...
async def create_consultation_thread(
    ...
    # Auto-register doctor in emergency contacts
    add_doctor_to_emergency_contacts(db, patient_id, provider.id)
...
async def send_consultation_message(
    ...
    # Auto-register doctor in emergency contacts
    add_doctor_to_emergency_contacts(db, thread.user_id, thread.provider_id)
